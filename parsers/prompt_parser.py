...
class PromptParser:
    ...
    def parse(self) -> ParsedPrompt:
        ...
        while i < len(self.lines):
            ...
            # ВАЖНО: во всех 3 форматах после закрывающих ** может сразу идти
            # описание роли (в этой же строке) — раньше эта часть ловилась в
            # "мусорную" группу и выбрасывалась. Теперь она попадает в
            # last group и используется как начало prompt_text (см. ниже).
            role = None
            
            # Формат 1: "1. **Роль 1 — Название** — описание (может быть в этой же строке)"
            m1 = re.match(r'^(\d+)\.\s+\*\*Роль\s+(\d+)\s+—\s+(.+?)\*\*\s*(.*)$', line)
            if m1:
                role_num = m1.group(2)
                role_name = m1.group(3).strip()
                same_line_tail = m1.group(4)
                role = self._parse_role_body(i, role_num, role_name, same_line_tail)
            
            # Формат 2: "**38. Роль 38 — Название** — описание" (двойная звёздочка в начале)
            if not role:
                m2 = re.match(r'^\*\*(\d+)\.\s+Роль\s+(\d+)\s+—\s+(.+?)\*\*\s*(.*)$', line)
                if m2:
                    role_num = m2.group(2)
                    role_name = m2.group(3).strip()
                    same_line_tail = m2.group(4)
                    role = self._parse_role_body(i, role_num, role_name, same_line_tail)
            
            # Формат 3: "**Роль 1 — Название** — описание" (без номера строки)
            if not role:
                m3 = re.match(r'^\s*\*\*Роль\s+(\d+)\s+—\s+(.+?)\*\*\s*(.*)$', line)
                if m3:
                    role_num = m3.group(1)
                    role_name = m3.group(2).strip()
                    same_line_tail = m3.group(3)
                    role = self._parse_role_body(i, role_num, role_name, same_line_tail)
    ...
    def _parse_role_body(self, start_idx: int, role_num: str, role_name: str, same_line_tail: str = "") -> ParsedRole:
        ...
        same_line_tail — текст, который шёл СРАЗУ ПОСЛЕ закрывающих ** в той же
        строке, что и заголовок роли (например: "1. **Роль 1 — Имя** — вот тут").
        Раньше этот текст просто отбрасывался, из-за чего роли, описанные одним
        абзацем в строке заголовка (а не списком строк ниже), получали пустой
        prompt_text. Теперь он идёт первой "строкой" тела роли.
        """
        full_name = f"Роль {role_num}: {role_name}"
        prompt_lines = []

        tail = (same_line_tail or "").strip()
        # убираем ведущее тире-разделитель ("— описание" → "описание")
