...
def _chunk_text(text: str, size: int = 4000) -> list:
...
async def cmd_roles(message: types.Message):
    ...
    async with async_session() as session:
        ...
        if has_records:
            ...
            allowed = ["lite"] if user.tariff == "lite" else ["lite", "pro"] if user.tariff == "pro" else ["lite", "pro", "business"]
            result = await session.execute(
                select(Role).where(Role.is_active == True, Role.tier_access.in_(allowed))
            )
            roles = result.scalars().all()
...
async def cmd_commands(message: types.Message):
    ...
    async with async_session() as session:
        allowed = ["lite"] if user.tariff == "lite" else ["lite", "pro"] if user.tariff == "pro" else ["lite", "pro", "business"]
        result = await session.execute(
            select(CommandModel).where(CommandModel.tier_access.in_(allowed))
        )
...
async def cmd_searchai(message: types.Message):
    ...
    answer, success = await ask_llm(
        prompt=query, user_id=message.from_user.id, model=default_model,
        system_prompt=system_prompt, max_tokens=1500
    )

    if success:
        try:
            tokens_used = count_tokens(query) + count_tokens(answer) + count_tokens(search_context) + count_tokens(system_prompt)
            async with async_session() as session:
                result = await session.execute(
...
async def smart_handler(message: types.Message):
    ...
    answer, success = await ask_llm(
        prompt=message.text, user_id=message.from_user.id, model=default_model,
        system_prompt=system_prompt, max_tokens=max_tokens
    )

    if success:
        try:
            tokens_used = count_tokens(message.text) + count_tokens(answer) + count_tokens(system_prompt)
            async with async_session() as session:
                result = await session.execute(
                    select(UserState).where(UserState.user_id == message.from_user.id)
                )
