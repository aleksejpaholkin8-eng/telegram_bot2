...
async def _search_duckduckgo(query: str, max_results: int = 5) -> Tuple[List[dict], str]:
    ...
    for attempt in range(2):
        try:
            with DDGS(headers={"User-Agent": "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36"}) as ddgs:
                results = list(ddgs.text(query, max_results=max_results, backend="lite"))
                if not results:
                    return [], "Ничего не найдено"
                return [{"title": r.get("title", "Без названия"), "url": r.get("href", ""), "snippet": r.get("body", "")} for r in results], ""
...
async def _search_searxng(query: str, max_results: int = 5) -> Tuple[List[dict], str]:
    ...
    for instance in instances:
        try:
            ...
            params = {"q": query, "format": "json", "categories": "general", "language": "ru-RU"}
            timeout = aiohttp.ClientTimeout(total=10)
            async with aiohttp.ClientSession(timeout=timeout) as session:
                async with session.get(url, params=params, headers={"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36"}) as response:
                    ...
                    if not results:
                        continue
                    return [{"title": r.get("title", "Без названия"), "url": r.get("url", ""), "snippet": r.get("content", "")} for r in results], ""
        except Exception as e:
...
async def web_search(query: str, max_results: int = 5) -> Tuple[List[dict], str]:
    results, _ = await _search_duckduckgo(query, max_results)
    if results:
        return results, ""
    logger.info("DuckDuckGo не сработал, пробуем SearXNG...")
    results, _ = await _search_searxng(query, max_results)
    if results:
        return results, ""
    return [], (
